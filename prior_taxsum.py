...
def build_summary_pdf(data, brackets, out_path):
    ...
    title_s = sty("T", parent=styles["Title"], fontSize=16, spaceAfter=4,
                  textColor=colors.HexColor("#1a3557"))
    sub_s   = sty("Sub", fontSize=9, textColor=colors.HexColor("#444444"), spaceAfter=6)
    h1 = sty("H1", parent=styles["Heading1"], fontSize=13,
             textColor=colors.HexColor("#1a3557"), spaceAfter=4, spaceBefore=10)
    h2 = sty("H2", parent=styles["Heading2"], fontSize=11,
             textColor=colors.HexColor("#2e5f8a"), spaceAfter=3, spaceBefore=8)
    small = sty("Sm", fontSize=8, textColor=colors.HexColor("#555555"))
    ...
    def section(elements):
        """Wrap a list of flowables in KeepTogether so sections stay on one page."""
    ...
    inc_rows = [
        ...
        ["Ordinary Dividends minus Qualified Dividends (line 3b - 3a)",
         fmt(non_qual), f"{pct(non_qual, ti):.1f}%"],
        ["Qualified Dividends (Form 1040 line 3a)",
         fmt(data["qual_div"]), f"{pct(data['qual_div'], ti):.1f}%"],
        ["IRA Withdrawal — Taxable (Form 1040 line 4b)",
         fmt(data["ira_taxable"]), f"{pct(data['ira_taxable'], ti):.1f}%"],
        ["Total Income (Form 1040 line 9)", fmt(ti), "100.0%"],
    ]
    inc_t = Table(inc_rows, colWidths=[3.5*inch, 1.6*inch, 1.7*inch])
    its = mk_ts()
    its.add("FONTNAME", (0, -1), (-1, -1), "Helvetica-Bold")
    its.add("BACKGROUND", (0, -1), (-1, -1), BLUE_LIGHT)
    inc_t.setStyle(its)
    section([Paragraph("Income Breakdown", h1), inc_t, Spacer(1, 8)])

    # ── TAX BREAKDOWN (QDW) ────────────────────────────────────
    ttq = data["qdw_line25"]
    tax_rows = [
        ["Category", "Income Amount", "Tax Amount", "% of Total Tax"],
        ["15% Tax Bracket (line 17 income / line 18 tax)",
         fmt(data["qdw_line17"]), fmt(data["qdw_line18"]),
         f"{pct(data['qdw_line18'], ttq):.1f}%"],
        ["20% Tax Bracket (line 20 income / line 21 tax)",
         fmt(data["qdw_line20"]), fmt(data["qdw_line21"]),
         f"{pct(data['qdw_line21'], ttq):.1f}%"],
        ["Ordinary Income (line 5 income / line 22 tax)",
         fmt(data["qdw_line5"]), fmt(data["qdw_line22"]),
         f"{pct(data['qdw_line22'], ttq):.1f}%"],
        ["Total Tax (line 25)", "", fmt(ttq), "100.0%"],
    ]
    tts = mk_ts()
    tts.add("FONTNAME",   (0, -1), (-1, -1), "Helvetica-Bold")
    tts.add("BACKGROUND", (0, -1), (-1, -1), BLUE_LIGHT)
    tax_t = Table(tax_rows, colWidths=[2.8*inch, 1.5*inch, 1.3*inch, 1.2*inch])
    tax_t.setStyle(tts)
    section([
        Paragraph("Tax Breakdown — Qualified Dividends &amp; Capital Gain Tax Worksheet", h1),
        Paragraph("All line references from QD &amp; Capital Gain Tax Worksheet. Total Tax = line 25.", small),
        Spacer(1, 4), tax_t, Spacer(1, 10),
    ])
    ...
    ord_t = Table(ord_rows, colWidths=[1.5*inch, 2.0*inch, 1.8*inch, 1.5*inch])
    ord_t.setStyle(ots)
    section([
    ...
    niit_t = Table(niit_rows, colWidths=[4.0*inch, 2.8*inch])
    niit_t.setStyle(mk_ts())
    section([Paragraph("Net Investment Income Tax (NIIT) — Form 8960", h1),
             niit_t, Spacer(1, 10)])

    # ── FOREIGN TAX CREDIT ─────────────────────────────────────
    ftc_val = data.get("foreign_tax_credit", 0)
    ftc_rows = [
        ["Item", "Amount"],
        ["Foreign Tax Credit (Schedule 3 line 1 / Form 1116 line 35)", fmt(ftc_val)],
    ]
    ftc_t = Table(ftc_rows, colWidths=[4.5*inch, 2.3*inch])
    ftc_t.setStyle(mk_ts())
    section([Paragraph("Foreign Tax Credit — Form 1116 / Schedule 3", h1),
             ftc_t, Spacer(1, 10)])

    # ── PAYMENT SCHEDULE ───────────────────────────────────────
    pay_rows = [
        ["", "Period 1", "Period 2", "Period 3", "Period 4"],
        ["Period (2210AI dates)"]  + data["ai_period_label"],
    ...
    pay_t = Table(pay_rows, colWidths=[1.6*inch, 1.3*inch, 1.3*inch, 1.3*inch, 1.3*inch])
    pay_t.setStyle(pts)
    section([
        Paragraph("Tax Payment Schedule — Form 2210 (Annualized Income Installment Method)", h1),
        Paragraph("Period dates: 1/1–3/31, 4/1–5/31, 6/1–8/31, 9/1–12/31. "
                  "Deadline dates per Form 2210 Part III. ES vouchers for next year not included.", small),
        Spacer(1, 4), pay_t, Spacer(1, 10),
    ])

    # ── REMAINING TAX / REFUND ─────────────────────────────────
    bal = data["balance_due"]
    label = "Refund" if bal < 0 else "Balance Due"
    rem_rows = [
        ["Item", "Amount"],
        [label, fmt(abs(bal))],
    ]
    rts = mk_ts()
    rts.add("FONTNAME",   (0, -1), (-1, -1), "Helvetica-Bold")
    rts.add("BACKGROUND", (0, -1), (-1, -1),
            colors.HexColor("#ffe0b2") if bal >= 0 else colors.HexColor("#c8e6c9"))
    rem_t = Table(rem_rows, colWidths=[4.0*inch, 2.8*inch])
    rem_t.setStyle(rts)
    section([Paragraph("Remaining Tax Owed / Refund", h1), rem_t])
